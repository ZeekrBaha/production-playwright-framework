...
test.describe("Activity trail", { tag: ["@workflow", "@regression"] }, () => {
  test("submitting records save and submit events", async ({
    pageFactory,
    seedScenarios,
  }) => {
    ...
    await expect(grid.activityItems.first()).toContainText(
      "ines submitted for review",
    );
    await expect(
      grid.activityItems.filter({ hasText: "saved changes" }),
    ).toHaveCount(1);
  });
});
...
test.describe("Activity trail — reviewer events", { tag: ["@workflow", "@regression"] }, () => {
  ...
  test("approval records a reviewer event", async ({
    pageFactory,
    seedScenarios,
  }) => {
