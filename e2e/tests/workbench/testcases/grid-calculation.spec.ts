...
test.describe("Forecast grid calculation", { tag: "@regression" }, () => {
  test(
    ...
    async ({ pageFactory, seedScenarios }) => {
      ...
      await expect(grid.value("netSales", "2026-01")).toHaveText(
        asCurrency(netSales),
      );
    },
  );

  test("row totals roll up across all months", async ({
    pageFactory,
    seedScenarios,
  }) => {
    ...
    await expect(grid.total("units")).toHaveText(
      expectedUnitsTotal.toLocaleString("en-US"),
    );
